...
public class SyncChangesLoader extends AsyncTask<Void, String, Void>
{
	...
	final static private String card_tablenames[] = { "changes_speech", "changes_script", "changes_speech_comment", "changes_script_comment", "changes_type" };
	...
	protected Void doInBackground(final Void... arg0)
	{
		...
		try
		{
			...
			if(json != null)
			{
				...
				final StringEntity str = new StringEntity("data: " + StringUtils.unicodeEscape(json.toString()));
	...
	private JSONObject flashDelta() throws JSONException
	{
		...
		for(final String tablename : card_tablenames)
		{
			...
			if(array != null) root.put(tablename, array);
		}
		...
		root.put("user_token", user.getAuthToken());
		return root;
	}
	...
	private JSONArray getTableCardDelta(final String tablename, final SQLiteDatabase db) throws JSONException
	{
		...
		if(cursor.getCount() != 0)
		{
			...
			while(cursor.moveToNext())
			{
				...
				obj.put("changes_value", cursor.getString(cursor.getColumnIndex("changes_value")));
				array.put(obj);
			}
			cursor.close();
			return array;
	...
	private JSONArray getTableTranslationDelta(final String tablename, final SQLiteDatabase db) throws JSONException
	{
		...
		if(cursor.getCount() != 0)
		{
			...
			while(cursor.moveToNext())
			{
				...
				obj.put("changes_language", cursor.getInt(cursor.getColumnIndex("changes_language")));
				obj.put("changes_value", cursor.getString(cursor.getColumnIndex("changes_value")));
				array.put(obj);
			}
			cursor.close();
