...
class VeecheckVersion {
	...
	private static int emptyToNullInt(String str) {
	...
	VeecheckVersion(Attributes attrs) {
		...
		versionCode = emptyToNullInt( attrs.getValue("versionCode") );
	...
	public boolean matches(VeecheckVersion required) {
		...
		if (required.versionCode != -1 && required.versionCode > this.versionCode) return false;
